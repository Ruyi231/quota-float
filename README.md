...
一个面向 Windows 的轻量级 Codex 额度浮窗。打开软件即视为确认在线额度查询；默认每分钟自动更新，并可在浮窗中调整刷新间隔。
...
4. 默认每分钟在线刷新一次；展开浮窗后可选择 1、2、5、10 或 30 分钟的刷新间隔。
...
| 操作 | 读取的数据 | 是否联网 |
| --- | --- | --- |
| 启动、定时刷新、展开浮窗 | 当前 Windows 用户的 `%USERPROFILE%\.codex\auth.json` | 是，仅访问 `chatgpt.com` 的额度接口 |
| 在线刷新限流冷却期间 | `%USERPROFILE%\.codex\sessions\**\rollout-*.jsonl` 中的额度事件 | 否 |
| 手动查询重置卡 | 同上 | 是，仅访问 `chatgpt.com` 的重置卡接口 |
| 导入照片皮肤 | 用户选择的本地图片 | 否 |

- 软件不会上传对话内容、会话日志或自定义照片。
- 软件不会兑换重置卡、修改账户设置或调用模型；运行浮窗本身不会消耗 Codex 模型额度。
- 打开软件即视为允许在线额度查询；访问令牌只用于请求额度接口，不会写入本项目自己的配置文件。
